...
# =========================================================
# 2. PANEL DE PERSONAL DINÁMICO
# =========================================================
def pantalla_personal_green():
    st.markdown("## 👥 Personal de Operaciones (Greenmovil)")
    st.info("💡 **Programación Individual:** Cada empleado tiene su propia zona y día de descanso base.")
    ...
    if archivo_personal is not None:
        if st.button("🔄 Importar Plantilla", key="btn_imp_pers_grn"):
            ...
            if "Nombre" in df_cargado.columns and "Cargo" in df_cargado.columns:
                ...
                df_cargado["Descanso Base"] = "Domingo" 
    ...
    if not df_pers.empty:
        if "Descanso Base" not in df_pers.columns: df_pers["Descanso Base"] = "Domingo"
...
def pantalla_parametrizador_green():
    ...
    st.info("Define las métricas operativas y las rotaciones.")
    ...
    with t_turnos:
        ...
        if df_turnos.empty:
            df_turnos = pd.DataFrame({
                ...
                "Cargo Aplicable": ["Técnico De Ejecucion De Operaciones", "Técnico De Ejecucion De Operaciones", "Técnico De Ejecucion De Operaciones", "Técnico De Ejecucion De Operaciones", "Operador SENIOR"],
    ...
    with t_rotaciones:
        ...
        st.caption("Asigna secuencias únicas. Si todos tienen Fase 1, todos tendrán el mismo turno. Usa el botón **Auto-Distribuir** para balancearlos.")
        
        df_pers_rot = cargar_tabla("green_personal")
        df_rot = cargar_tabla("green_rotaciones_ind")
        
        if df_pers_rot.empty:
            st.warning("No hay personal registrado.")
        else:
            rot_data = []
            for _, row_p in df_pers_rot.iterrows():
                n = row_p["Nombre"]
                c = row_p["Cargo"]
                z = row_p.get("Zona", "General")
                
                match = df_rot[df_rot["Nombre"] == n] if not df_rot.empty else pd.DataFrame()
                if not match.empty:
                    rot_data.append({
                        "Zona": z, "Cargo": c, "Nombre": n, 
                        "Patrón de Rotación Semanal": match.iloc[0]["Patrón de Rotación Semanal"], 
                        "Fase Inicio (Semana)": match.iloc[0].get("Fase Inicio (Semana)", 1)
                    })
                else:
                    patron_def = "TC3, TC2, TC1, Inspector Electromovilidad" if "Tecnico" in str(c) else "Fijo"
                    rot_data.append({
                        "Zona": z, "Cargo": c, "Nombre": n, 
                        "Patrón de Rotación Semanal": patron_def, 
                        "Fase Inicio (Semana)": 1
                    })
            ...
            df_edit_rot = st.data_editor(
                ...
                column_config={
                    ...
                    "Fase Inicio (Semana)": st.column_config.NumberColumn("Semana de arranque (Fase)", min_value=1, required=True)
                }, key="edit_rot_ind_green"
            )
            
            c1_btn, c2_btn = st.columns(2)
            if c1_btn.button("💾 Guardar Patrones Individuales", key="btn_guar_rot_ind"):
                guardar_tabla(df_edit_rot, "green_rotaciones_ind")
                st.success("✅ Secuencias individuales guardadas.")
                
            if c2_btn.button("⚖️ Auto-Distribuir Fases (Evitar Choques)", key="btn_auto_fases"):
                # Inteligencia de balanceo
                df_a_distribuir = df_edit_rot.copy()
                contador = {}
                for idx, row in df_a_distribuir.iterrows():
                    llave = f"{row['Zona']}_{row['Cargo']}"
                    if llave not in contador: contador[llave] = 0
                    
                    patron_lista = [x.strip() for x in str(row["Patrón de Rotación Semanal"]).split(",")]
                    num_fases = len(patron_lista) if len(patron_lista) > 0 else 1
                    
                    fase_asignada = (contador[llave] % num_fases) + 1
                    df_a_distribuir.at[idx, "Fase Inicio (Semana)"] = fase_asignada
                    contador[llave] += 1
                    
                guardar_tabla(df_a_distribuir, "green_rotaciones_ind")
                st.success("✅ ¡Fases re-calculadas automáticamente! Vuelve a generar la malla.")
                st.rerun()
